...
public class TaskTests {
    ...
    @BeforeEach
    void setup() {
        project = new SoftwareProject("P001","Beta Portal", "Dynamic team validation sandbox", 45000000.00);
    ...
    void testDynamicTeamSizeIncrementOnJoin() {
        ...
        User employee1 = new RegularUser("Katy Great Adonai", "katygreatado@gmail.com", "password");
        project.addMember(employee1);
        assertEquals(1, project.getTeamSize(), "Team size did not dynamically update to 1 member.");

        User employee2 = new AdminUser("Aline NZIKWINKUNDA", "nzikaline@gmail.com", "password");
    ...
    void testAddDuplicateMemberThrowsInvalidInputException() {
        User duplicateUser = new RegularUser("Katy Great Adonai", "katygreatado@gmail.com", "password");
    ...
    void testNonExistingTaskThrowsTaskNotFoundException() {
        Task task = new Task("T001","Setup Database", Status.IN_PROGRESS);
        project.addTask(task);
        User user = new AdminUser("Aline NZIKWINKUNDA", "nzikaline@gmail.com", "password");
        TaskNotFoundException exception = assertThrows(TaskNotFoundException.class, () -> {
            taskService.updateTaskStatus(project, "NKLN", Status.COMPLETED.name(), user);
        });
